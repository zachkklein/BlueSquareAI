# pipeline/counterfactual_async.py
"""
Async counterfactual reasoning module with optimized parsing.
"""
...
async def counterfactual_test_async(claim: str) -> dict:
    """Test whether a claim relies on implicit identity-based meaning using counterfactual reasoning (async version).
    ...
    try:
        response = await llm.ainvoke(
            prompt.format_messages(claim=claim)
        )
        ...
        return {
            ...
            "meaning_preserved": True,  # Default to preserved (lower risk)
            "explanation": f"Unable to determine due to error: {str(e)}",
            "error": str(e),
            "raw_output": str(e)
        }
...
def counterfactual_test(claim: str) -> dict:
    """Test whether a claim relies on implicit identity-based meaning using counterfactual reasoning (synchronous wrapper).
    ...
    """
    import asyncio
    return asyncio.run(counterfactual_test_async(claim))
